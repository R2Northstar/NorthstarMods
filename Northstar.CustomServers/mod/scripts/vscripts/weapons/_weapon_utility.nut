...
const float EMP_SEVERITY_SLOWMOVE = 0.5
const float LASER_STUN_SEVERITY_SLOWTURN = 0.2
const float LASER_STUN_SEVERITY_SLOWMOVE = 0.3
...
function WeaponUtility_Init()
{
	...
	void function EntitiesDidLoad()
	{
			...
			// float titanRocketLauncherInnerRadius = expect float( GetWeaponInfoFileKeyField_Global(
			// "mp_titanweapon_rocketeer_rocketstream", "explosion_inner_radius" ) )
...
void function OnWeaponActivate_updateViewmodelAmmo( entity weapon )
{
	...
	void function Defender_DamagedPlayerOrNPC( entity ent, var damageInfo )
	{
		...
		local damage = Vortex_HandleElectricDamage(
			ent,
	...
	void function SMR_DamagedPlayerOrNPC( entity ent, var damageInfo )
	{
		...
		if ( IsValid( attacker ) && attacker.IsPlayer() && attacker.GetTitanSoulBeingRodeoed() == ent.GetTitanSoul() )
			attacker.TakeDamage(
				...
				attacker,
				attacker,
				{ scriptType = DF_GIB | DF_EXPLOSION, damageSourceId = eDamageSourceId.mp_weapon_smr, weapon = DamageInfo_GetWeapon( damageInfo ) }
			)
	}
...
*/
	...
	array<VisibleEntityInCone> results = FindVisibleEntitiesInCone(
		pos,
		dir,
		maxDistance,
		( maxAngle * 1.1 ),
...
int function ShotgunBlastDamageEntity( entity weapon, vector barrelPos, vector barrelVec, VisibleEntityInCone result, float angle, float maxAngle, int numPellets, int damageType, float damageScaler )
{
	...
	weapon.FireWeaponBullet_Special( barrelPos, vecToEnt, 1, damageType, true, true, true, false, false, false, false ) // fires perfect bullet with no antilag and no spread
		...
		// if ( angle > 0 )
		// 	coneScaler = GraphCapped( angle, (maxAngle * SHOTGUN_ANGLE_MIN_FRACTION), (maxAngle *
		// SHOTGUN_ANGLE_MAX_FRACTION), SHOTGUN_DAMAGE_SCALE_AT_MIN_ANGLE, SHOTGUN_DAMAGE_SCALE_AT_MAX_ANGLE )

		// Calculate the final damage abount to inflict on the target. Also scale it by damageScaler which may
		// have been passed in by script ( used by alt fire mode on titan shotgun to fire multiple shells )
		float finalDamageAmount = damageAmount * coneScaler * damageScaler
		// printt( "angle:", angle, "- coneScaler:", coneScaler, "- damageAmount:", damageAmount,
		// "- damageScaler:", damageScaler, "  = finalDamageAmount:", finalDamageAmount )
...
{
	entity missile = weapon.FireWeaponMissile(
		attackParams.pos,
		attackParams.dir,
		1,
		damageTypes.largeCaliberExp,
		damageTypes.largeCaliberExp,
		false,
...
function DetonateAllPlantedExplosives( entity player )
{
	...
	foreach ( grenade in grenades )
	{
		...
		if (
			grenade.ProjectileGetDamageSourceID() != eDamageSourceId.mp_weapon_satchel &&
			grenade.ProjectileGetDamageSourceID() != eDamageSourceId.mp_weapon_proximity_mine
		)
			continue
...
function IsValidSatchel( entity satchel )
{
	...
	table function GetBulletPassThroughTargets( entity attacker, WeaponBulletHitParams hitParams )
	{
		...
		table passThroughInfo = { endPos = null, targetArray = [] }
	...
	function StartClusterExplosions( entity projectile, entity owner, PopcornInfo popcornInfo, customFxTable = null )
	{
		...
		foreach ( player in players )
		{
			Remote_CallFunction_NonReplay(
				...
				owner.GetTeam(),
				owner.GetEncodedEHandle(),
				placementHelper.GetEncodedEHandle(),
				outerRadius
			)
		}
...
{
	local missileVecs = GetExpandContractRocketTrajectories(
		weapon,
		attackParams.burstIndex,
		attackPos,
		attackDir,
		rocketsPerShot,
		launchOutAng,
		launchInAng,
		burstFireCountOverride
	)
	entity owner = weapon.GetWeaponOwner()
...
*/
			...
			missile.InitMissileExpandContract(
				missileVecs[ i ].outward,
				missileVecs[ i ].inward,
				launchOutTime,
				launchInLerpTime,
				launchInTime,
				launchStraightLerpTime,
				missileEndPos,
				applyRandSpread
			)
...
float function ChargeBall_GetChargeTime()
{
	...
	void function GivePlayerAmpedWeapon( entity player, string weaponName )
	{
		...
		// This is more complicated than it should be because of rules of what weapons can be in
		// what slots, e.g.  your anti-titan weapon can't be replaced by non anti-titan weapons
	...
	void function WeaponAttackWave( entity ent, int projectileCount, entity inflictor, vector pos, vector dir, bool functionref( entity, int, entity, entity, vector, vector, int ) waveFunc )
	{
		...
		for ( int i = 0; i < maxCount; i++ )
		{
			...
			if ( forwardTrace.fraction == 1.0 )
			{
				...
				TraceResults downTrace = TraceLine(
					forwardTrace.endPos,
					forwardTrace.endPos + < 0.0, 0.0, -1000.0 >,
					ignoreArray,
					TRACE_MASK_SHOT,
			...
			{
				if (
					IsValid( forwardTrace.hitEnt ) && ( StatusEffect_Get( forwardTrace.hitEnt, eStatusEffect.pass_through_amps_weapon ) > 0 ) &&
					!CheckPassThroughDir( forwardTrace.hitEnt, forwardTrace.surfaceNormal, forwardTrace.endPos )
				)
					break
			}
			...
			else
			{
				TraceResults downTrace = TraceLine(
					upwardTrace.endPos,
					upwardTrace.endPos + < 0.0, 0.0, -1000.0 >,
					ignoreArray,
					TRACE_MASK_SHOT,
					TRACE_COLLISION_GROUP_BLOCK_WEAPONS
				)
	...
	void function VanguardEnergySiphon_DamagedPlayerOrNPC( entity ent, var damageInfo )
	{
		...
		Elecriticy_DamagedPlayerOrNPC(
			ent,
			damageInfo,
			FX_VANGUARD_ENERGY_BODY_HUMAN,
			FX_VANGUARD_ENERGY_BODY_TITAN,
			LASER_STUN_SEVERITY_SLOWTURN,
			LASER_STUN_SEVERITY_SLOWMOVE
		)
	}
...
*/
	...
	function EMPGrenade_EffectsPlayer( entity player, damageInfo )
	{
		...
		float duration = EMP_GRENADE_PILOT_SCREEN_EFFECTS_DURATION_MIN +
			( ( EMP_GRENADE_PILOT_SCREEN_EFFECTS_DURATION_MAX - EMP_GRENADE_PILOT_SCREEN_EFFECTS_DURATION_MIN ) * frac ) - fadeoutDuration
