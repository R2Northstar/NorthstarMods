...
void function CleanUpEntitiesForRoundEnd()
{
	...
	foreach ( entity player in GetPlayerArray() )
	{
		...
		if ( IsAlive( player ) )
			player.Die( svGlobal.worldspawn, svGlobal.worldspawn, { scriptType = DF_GIB, damageSourceId = eDamageSourceId.round_end } )
	}
	...
	foreach ( entity npc in GetNPCArray() )
	{
		...
		npc.Die( svGlobal.worldspawn, svGlobal.worldspawn, { scriptType = DF_DISSOLVE | DF_RAGDOLL, damageSourceId = eDamageSourceId.round_end } )
	}
