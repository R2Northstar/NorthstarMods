...
void function InitUnlocks()
{
	...
	for ( int i = 0; i < numRows; i++ )
	{
		...
		// InitUnlock( GetDataTableString( titanLoadoutsDataTable, i, GetDataTableColumnByName(
		// titanLoadoutsDataTable, "passive1" ) ), titanRef, eUnlockType.TITAN_LEVEL, 1 )
		// InitUnlock( GetDataTableString( titanLoadoutsDataTable, i, GetDataTableColumnByName(
		// titanLoadoutsDataTable, "passive2" ) ), titanRef, eUnlockType.TITAN_LEVEL, 1 )
		// InitUnlock( GetDataTableString( titanLoadoutsDataTable, i, GetDataTableColumnByName(
		// titanLoadoutsDataTable, "passive3" ) ), titanRef, eUnlockType.TITAN_LEVEL, 1 )
	}
...
void function InitUnlock( string ref, string parentRef, int unlockType, int unlockLevel, int cost = 0, string additionalRef = "" )
{
	...
	if ( parentRef == "" )
	{
		...
		Assert(
			unlockType == eUnlockType.PLAYER_LEVEL || unlockType == eUnlockType.DISABLED || unlockType == eUnlockType.ENTITLEMENT ||
				unlockType == eUnlockType.PERSISTENT_ITEM || unlockType == eUnlockType.FACTION_LEVEL || unlockType == eUnlockType.FD_UNLOCK_POINTS,
			"Cannot specify a parentRef for an item that unlocks with player level"
		)
...
{
	if ( iType == eItemTypes.PILOT_PRIMARY || iType == eItemTypes.PILOT_SECONDARY || iType == eItemTypes.TITAN_PRIMARY )
		return true
...
{
	if ( itemType == eItemTypes.PILOT_PRIMARY_MOD || itemType == eItemTypes.PILOT_SECONDARY_MOD || itemType == eItemTypes.TITAN_PRIMARY_MOD )
		return true
...
bool function IsItemNew( entity player, string ref, string parentRef = "" )
{
	...
	{
		if (
			HasAnyNewItemOfType( player, eItemTypes.PILOT_PRIMARY ) || HasAnyNewItemOfType( player, eItemTypes.PILOT_SECONDARY ) ||
			HasAnyNewItemOfType( player, eItemTypes.PILOT_ORDNANCE ) || HasAnyNewItemOfType( player, eItemTypes.PILOT_PASSIVE1 ) ||
			HasAnyNewItemOfType( player, eItemTypes.PILOT_PASSIVE2 ) || HasAnyNewItemOfType( player, eItemTypes.PILOT_SUIT ) ||
			HasAnyNewItemOfType( player, eItemTypes.PILOT_EXECUTION ) || HasAnyNewItemOfType( player, eItemTypes.CAMO_SKIN_PILOT ) || HasAnyNewPilotLoadout( player )
		)
			return true
	...
	{
		if ( HasAnyNewItemOfType( player, eItemTypes.TITAN ) || HasAnyNewItemOfType( player, eItemTypes.PRIME_TITAN ) )
			return true
	...
	{
		if (
			HasAnyNewItemOfType( player, eItemTypes.BURN_METER_REWARD ) || HasAnyNewItemOfType( player, eItemTypes.FACTION ) ||
			HasAnyNewItemOfType( player, eItemTypes.CALLING_CARD ) || HasAnyNewItemOfType( player, eItemTypes.CALLSIGN_ICON )
		)
			return true
		...
		switch ( itemType )
		{
			...
			case eItemTypes.PILOT_SECONDARY:
				if ( menu == "EditPilotLoadoutsMenu" )
					return HasAnyNewItemOfType( player, itemType, -1, ref ) || RefHasAnyNewSubitem( player, ref, eItemTypes.CAMO_SKIN ) ||
						RefHasAnyNewSubitem( player, ref, eItemTypes.WEAPON_SKIN )
				else if (
					menu == "EditPilotLoadoutMenu" || menu == "EditTitanLoadoutsMenu" || menu == "EditTitanLoadoutMenu" || menu == "PilotLoadoutsMenu" ||
					menu == "TitanLoadoutsMenu"
				)
					return HasAnyNewItemOfType( player, itemType, -1, ref )
				else
					return (
						IsItemNew( player, ref ) || RefHasAnyNewSubitem( player, ref, eItemTypes.PILOT_PRIMARY_ATTACHMENT ) ||
							RefHasAnyNewSubitem( player, ref, eItemTypes.PILOT_PRIMARY_MOD ) || RefHasAnyNewSubitem( player, ref, eItemTypes.PILOT_SECONDARY_MOD ) ||
							RefHasAnyNewSubitem( player, ref, eItemTypes.PILOT_WEAPON_MOD3 ) || RefHasAnyNewSubitem( player, ref, eItemTypes.SUB_PILOT_WEAPON_MOD ) ||
			...
			case eItemTypes.TITAN_VANGUARD_EXECUTION:
				if (
					menu == "EditPilotLoadoutsMenu" || menu == "EditPilotLoadoutMenu" || menu == "EditTitanLoadoutsMenu" || menu == "EditTitanLoadoutMenu" ||
					menu == "PilotLoadoutsMenu" || menu == "TitanLoadoutsMenu" || menu == "ArmoryMenu"
				)
...
{
	Assert( gender == "race_human_male" || gender == "race_human_female" )
...
{
	Assert(
		loadoutProperty == "passive1" || loadoutProperty == "passive2" || loadoutProperty == "passive3" || loadoutProperty == "passive4" ||
			loadoutProperty == "passive5" || loadoutProperty == "passive6"
	)
...
{
	Assert(
		propertyName == "primaryAttachment" || propertyName == "primaryMod1" || propertyName == "primaryMod2" || propertyName == "primaryMod3" ||
			propertyName == "secondaryMod1" || propertyName == "secondaryMod2" || propertyName == "secondaryMod3" || propertyName == "weapon3Mod1" ||
			propertyName == "weapon3Mod2" || propertyName == "weapon3Mod3"
	)
