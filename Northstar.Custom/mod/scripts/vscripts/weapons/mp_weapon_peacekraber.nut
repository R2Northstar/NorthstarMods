...
{
	#if CLIENT
		if ( !weapon.GetWeaponOwner().IsPlayer() || weapon.GetWeaponOwner() != GetLocalViewPlayer() )
			return
		isWeaponActive = true
		clientWeapon = weapon
			...
			switch ( chargeLevel )
			{
				...
				case 1:
					if ( chargeFrac > 0.6 )
					{
						...
						RuiSetFloat( rui, "adjustedSpread", 0.065 )
					break
				...
				case 2:
					if ( chargeFrac > 0.933 )
					{
						...
						RuiSetFloat( rui, "adjustedSpread", 0.045 )
					break
...
function FireWeaponPlayerAndNPC( WeaponPrimaryAttackParams attackParams, bool playerFired, entity weapon )
{
	...
	if ( shouldCreateProjectile )
	{
		...
		for ( int index = 0; index < PEACEKRABER_MAX_BOLTS; index++ )
		{
			...
			entity bolt = weapon.FireWeaponBolt(
				attackParams.pos,
				attackDir,
				projectileSpeed,
				damageTypes.largeCaliber | DF_SHOTGUN,
				damageTypes.largeCaliber | DF_SHOTGUN,
				playerFired,
				index
			)
			if ( bolt )
			{
				...
				if ( weapon.GetWeaponClassName() == "mp_weapon_peacekraber" )
					...
					bolt.SetProjectileLifetime( RandomFloatRange( 0.5, 0.65 ) )
			}
		}
	}
