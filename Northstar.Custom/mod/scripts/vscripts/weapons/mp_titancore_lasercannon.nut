...
void function LaserCannon_Init()
{
	...
	void function LaserCore_OnPlayedOrNPCKilled( entity victim, entity attacker, var damageInfo )
	{
		...
		if ( !IsValid( soul ) || attacker.GetPlayerNetInt( EARNMETER_MODE ) != eEarnMeterMode.CORE_ACTIVE )
			return
...
void function OnAbilityEnd_LaserCannon( entity weapon )
{
	...
	void function LaserEndingWarningSound( entity weapon, entity player )
	{
		...
		// 	Assert( duration > 2.0, "Titan_Core_Laser_Fire_EndWarning_1P needs to be played 2.0
		// seconds before. Ask audio to adjust the sound and change the values in this function" )
		// 	wait duration - 2.0
