...
void function InitInGameMPMenu()
{
    ...
    AddMenuFooterOption( menu, BUTTON_Y, "%[Y_BUTTON]% " + Localize( "#SHOW_MATCH_INFO" ), "#SHOW_MATCH_INFO", void function( var button ){ AdvanceMenu( GetMenu( "MatchInfo" ) ) } )
}
...
{
    string serverId = "#INFO_UNAVAILABLE"
    string serverName = "#INFO_UNAVAILABLE"
    string serverDesc = "#INFO_UNAVAILABLE"
    ...
    var infoPanel = Hud_GetChild( file.menuMP, "ServerInfo" )
    Hud_SetText( Hud_GetChild( infoPanel, "ServerName" ), serverName )
    Hud_SetText( Hud_GetChild( infoPanel, "ServerDescription" ), serverDesc )
    Hud_SetText( Hud_GetChild( infoPanel, "MaxPlayers" ), Localize( "#INFO_SERVER_PLAYERS", serverMaxPlayers ) )

    SpinOff( void function() : ( infoPanel )
        {
            while( true )
            {
                Hud_SetText( Hud_GetChild( infoPanel, "PlayerPing" ), Localize( "#INFO_SERVER_PING", string( MyPing() ) ) )
                WaitFrame()
            }
        }
    )
