...
void function OnPlayButtonClick( var button )
{
	...
	thread StartFDMatchmaking( selectedDifficulties )
	//ClientCommand( "StartPrivateMatchSearch" )
}

void function StartFDMatchmaking( array<string> selectedDifficulties )
{
	...
	array<ServerInfo> FDServers
	array<ServerInfo> filteredServers
	
	foreach( index, string playlist in selectedDifficulties )
	{
		foreach ( ServerInfo server in servers )
		{
			if ( server.playlist == playlist )
				FDServers.append( server )
		}
	}
	
	foreach ( ServerInfo server in FDServers )
	{
	...
	{
		foreach ( ServerInfo server in FDServers ) //If we failed to find servers with players in it, consider joining empty ones
		{
			if ( server.playerCount == server.maxPlayerCount )
				continue;
